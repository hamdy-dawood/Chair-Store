import 'package:chair_store/modules/TapBar/Details%20Screen/card_details.dart';
import 'package:flutter/material.dart';
import 'armchair_card.dart';
import 'armchair_list.dart';

class ArmChair_screen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFFF5EFF1),
      body: SingleChildScrollView(
        scrollDirection: Axis.vertical,
        child: Column(
          children: [
            SizedBox(
                // height: MediaQuery.of(context).size.height,
                height: 700,
                child: ListView.builder(
                  itemCount: armchairs.length,
                  itemBuilder: (context, index) => ArmchairCard(
                    ItemIndex: index,
                    armchair: armchairs[index],
                    press: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => CardDetails(
                            details: armchairs[index],
                          ),
                        ),
                      );
                    },
                  ),
                )),
          ],
        ),
      ),
    );
  }
}
